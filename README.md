...
- Все ID вместо GUID, который по сути string, перевести на голые цифры (number), которые получаем от базы данных - ?
- Проверить смену state во всех сценах. Там уже появилось дублирующее поле зачем-то - ?
- На сервере должен быть список текущих онлайн пользователей, где будут оба сокета - ?
- Нужно добавить таймер в игру на сервере и клиенте - каждая игра начинается с -3 секунд и заканчивается после, например, минуты (потом можно сделать дольше, сейчас не до веселья). Для этого надо вместе с сообщением о координатах посылать и текущий time-code игры. В случае ровного счета - продлевать игру, пока кто-то не забьет - ?
...
- Когда одна игра закончилась:
	...
	- Если игроков <= 2:
		...
		- послать всем в сокет игры сообщение о переходе в обычный режим (все кнопки активны, в окне игры)
