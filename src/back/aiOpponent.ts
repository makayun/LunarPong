...
export class AIOpponent {
  ...
  private commandQueue: Array<typeof UP | typeof DOWN | typeof STOP> = [];
  private commandInterval: number = 100; // каждые 100 мс

  constructor(game: Game) {
    this.game = game;
    this.config = {
      paddleSpeed: PADDLE_STEP,
      updateInterval: 1000, // Одна стратегия в секунду
    };
  }
  ...
  update(positions: MeshPositions, currentTime: number): PlayerInput | null {
    if (currentTime - this.lastUpdate >= this.config.updateInterval) {
      this.lastUpdate = currentTime;
      this.computeDirectionQueue(positions);
    }

    if (currentTime - this.lastCommandTime >= this.commandInterval) {
  ...
  private computeDirectionQueue(positions: MeshPositions): void {
    this.commandQueue = [];
    ...
    if (predictedZ > paddleZ + threshold) {
      this.commandQueue = Array(steps).fill(UP);
    } else if (predictedZ < paddleZ - threshold) {
      this.commandQueue = Array(steps).fill(DOWN);
    } else {
    ...
    if (this.ballVelocity.x > 0) {
      ...
      const timeToPaddle =
        this.ballVelocity.x !== 0
          ? distanceToPaddle / this.ballVelocity.x
          : Infinity;
